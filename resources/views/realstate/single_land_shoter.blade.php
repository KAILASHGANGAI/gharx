...
<div class="text-center col-sm-6 p-0">
    <section class="container">
     <div class="full-img">
       ...
       <img id="expandedImg" src="{{asset($data->images)}}">
       <div id="imgtext"></div>
     </div>
<h2 class=" text-center mt-2">Price  :{{$data->price}} /-</h2>
...
<h2 class="display-5 py-3 single-heading">This {{$data->type}} is on {{$data->for}} At {{ucfirst($data->address_of_land)}}.</h2>
    <h4>Area : {{$data->area}} ( length * breadth )</h4>
    <h6>Surrounding</h6>
<ol>
        <li>East : {{$data->east}} ,</li>
        <li>West : {{$data->west}}</li>
        <li>North : {{$data->north}}</li>
        <li> South : {{$data->south}}.</li>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
       <form action="/apply_land" method="post">
       @csrf
         <label for="apply_land">Full Address</label>
         <input type="text" name="address" id="">
         <input type="hidden" name="id" value="{{base64_encode($data->id)}}">
         <button class="btn btn-success">Finally Apply</button>
       </form>
      </div>
